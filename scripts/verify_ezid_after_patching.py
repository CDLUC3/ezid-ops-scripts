...
def _create_identifier(base_url, user, password, status='reserved'):
    ...
    data = f"_status: {status}\n".encode("UTF-8") + toAnvl(record).encode("UTF-8")
...
def verify_reserve_and_delete_identifier(user, password, base_url):
    ...
    shoulder, id_created, text = _create_identifier(base_url, user, password, status='reserved')
    if id_created:
...
def verify_status_transitions_for_identifier(user, password, base_url):
    ...
    http_success, status_code, text, err_msg = post_data(url, user, password, f"_status: public\n".encode("UTF-8") + data)
    ...
    http_success, status_code, text, err_msg = post_data(url, user, password,
...
def verify_prefix_matching(user, password, base_url):
    ...
    shoulder, id_created, text = _create_identifier(base_url, user, password, status='public')
    if not id_created:
...
def main():
    ...
    verify_one_time_login(base_url, user, password)
    ...
    verify_reserve_and_delete_identifier(user, password, base_url)

    verify_status_transitions_for_identifier(user, password, base_url)

    verify_create_or_update_identifier(user, password, base_url)

    verify_prefix_matching(user, password, base_url)

    verify_introspection(user, password, base_url)

    check_background_jobs(env)

    check_batch_download(user, password, base_url, notify_email)

    check_resolver(base_url)
